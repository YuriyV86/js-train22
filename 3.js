...
class Store {
  ...
  unsubscribe(customer) {
    this.customers = this.customers.filter((subscriber) => subscriber !== customer);
  }
  ...
  notifyCustomers(product) {
    this.customers.forEach((subscriber) => subscriber.sendEmail(`Новий продукт "${product.name}" в магазині ${this.name}!`));
    }
}
